<?php
    ...
    $query = "SELECT * FROM `seminar_table` WHERE `id`='" . $post_id . "'";
    $result = mysqli_fetch_array(mysqli_query($connec, $query));
?>

<head>
    <title>Edit Seminar - CSI Admin</title>
    <link rel="stylesheet" href="../assets/styles.css">
</head>
<?php include('../layout/header.php') ?>
<div class="container mt-4 ml-auto mr-auto edit">
    <h3 class="text-center">Edit</h3>
    <form id="blog_form" class="edit_seminar_form">
        <div class="form-group">
            <label for="title">Seminar Title: </label>
            <input id="title" type="text" name="title" class="form-control" required placeholder="Enter a post title" autocomplete="off" value="<?php echo $result['seminar_title']; ?>">
        </div>
        <div class="form-group">
            <label for="content">Seminar Content: </label>
            <textarea name="content" id="editor" class="form-control"><?php echo $result['seminar_content']; ?></textarea>
        </div>
        <div class="form-group">
            <label for="written_by">Speaker: </label>
            <input type="text" id="author" name="written_by" class="form-control" value="<?php echo $result['seminar_speaker']; ?>">">
        </div>
